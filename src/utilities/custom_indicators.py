...
class Choppiness:
    ...
    def chop(high, low, close, window=14):
        ...
        chop_serie = (
            100
            * np.log10((atr.rolling(window).sum()) / (highh - lowl))
            / np.log10(window)
        )
...
class Trix:
    ...
    def __init__(self, close: pd.Series, trixLength: int = 9, trixSignal: int = 21):
        self.close = close
...
class VMC:
    ...
    def _run(self) -> None:
        ...
        self._esa = ta.trend.ema_indicator(close=self.hlc3, window=self._wtChannelLen)
    ...
    def money_flow(self) -> pd.Series:
        ...
        """
        mfi = (
            (self._close - self._open) / (self._high - self._low)
        ) * self._rsiMFIMultiplier
...
def volume_anomality(df, volume_window=10):
    ...
    dfInd.loc[dfInd["PreviousClose"] > dfInd["close"], "VolAnomaly"] = (-1) * dfInd[
        "VolAnomaly"
    ]
...
class SuperTrend:
    def __init__(self, high, low, close, atr_window=10, atr_multi=3):
        self.high = high
        self.low = low
        self.close = close
    ...
    def _run(self):
        ...
        atr = true_range.ewm(
            alpha=1 / self.atr_window, min_periods=self.atr_window
        ).mean()
        ...
        for i in range(1, len(self.close)):
            ...
            # else, the trend continues
                ...
                if (supertrend[curr]) and (
                    final_lowerband[curr] < final_lowerband[prev]
                ):
                    final_lowerband[curr] = final_lowerband[prev]
                if (not supertrend[curr]) and (
...
class MaSlope:
    ...
    def _run(self):
        ...
        for i in range(1, len(df)):
            ma1 = col_ma[i - 1]
            col_ma.append(
                ma1 + (df.iloc[i]["final"] ** 2) * (df.iloc[i]["close"] - ma1)
            )
